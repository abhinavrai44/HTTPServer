...
def gps():
	...
	with serial.Serial(addr,9600) as pt:
    		...
    		spb.readline()
		while (1):
        		cor = spb.readline()  # read one line of text from serial port
	return
...
class CamHandler(BaseHTTPRequestHandler):
	def do_GET(self):
		...
		if (self.path == "/cam.mjpg"):
			...
			self.send_response(200)
			faceCascade = cv2.CascadeClassifier("/home/pi/opencv-2.4.10/data/lbpcascades/lbpcascade_frontalface.xml")
			self.send_header('Content-type','multipart/x-mixed-replace; boundary=--jpgboundary')
			self.end_headers()
			t = clock()
		...
		if (self.path == "/record_end/cam.mjpg"):
			...
			while (True):
				...
				if(temp != whole_thing):
					...
					self.send_header('Content-length',str(len(whole_thing)-2))
					self.end_headers()
					self.wfile.write(whole_thing)
					#self.wfile.write("--aaboundary\r\n")
					self.wfile.write("\r\n\r\n\r\n\r\n")
...
if __name__ == '__main__':
	camera_thread = Thread(target=vid)
	camera_thread.start()
	main()
